...
void DISPLAY_Class::write(uint8_t display, uint8_t value) {
  ...
  if (value < 10)
  {
    uint8_t aux = (display == DISPLAY_U) ? 8 : (display == DISPLAY_D) ? 0 : 0;
    uint16_t display_value = ioexpander.outValue();
    display_value &= (0xFF00 >> aux); // Limpia los bits del display correspondiente
    display_value |= (dv[value] << aux); // Establece el valor del dígito en el display correspondiente
    ioexpander.writeAll(display_value);
  }
}
...
void DISPLAY_Class::writeF(float value) {
  const uint8_t dv[10] = {63, 6, 91, 79, 102, 109, 125, 7, 127, 111};
  if (value < 10.0f) {
    uint8_t unit = (uint8_t)(value);
    uint8_t dec_float = (value - (float)unit) * 10.0f; // Obtiene la parte decimal
    uint16_t display_value = ioexpander.outValue();
    display_value = dv[unit] | (dv[dec_float] << 8) | (1 << 7); // Asigna los valores de decenas y unidades
    ioexpander.writeAll(display_value);
  }
}

void DISPLAY_Class::dp(uint8_t display, uint8_t value) {
  uint8_t aux = (display == 2) ? segDP : ((display == 1) ? segDP: 0);
  uint16_t display_value = ioexpander.outValue();
  bitWrite(display_value, aux, value);
  ioexpander.writeAll(display_value);
}

void DISPLAY_Class::segmentU(uint8_t display, uint8_t segment, uint8_t value) {
  uint8_t aux = (display == 2) ? 8 : 0;
  uint16_t display_value = ioexpander.outValue();
...
void DISPLAY_Class::segmentM(uint8_t display, uint8_t A_SEGMENT, uint8_t B_SEGMENT, uint8_t C_SEGMENT, uint8_t D_SEGMENT, uint8_t E_SEGMENT, uint8_t F_SEGMENT,uint8_t G_SEGMENT, uint8_t DP){
  uint8_t aux = (display == 2) ? 8 : 0;
  uint16_t display_value = ioexpander.outValue();
...
void DISPLAY_Class::segmentB(uint8_t display, uint8_t value) {
  uint8_t aux = (display == 2) ? 8 : 0;
  uint16_t display_value = ioexpander.outValue();
  display_value &= (0xFF00 >> aux); // Limpia los bits del display correspondiente
  display_value |= (value << aux); // Establece el valor binario en el display correspondiente
  ioexpander.writeAll(display_value);
}